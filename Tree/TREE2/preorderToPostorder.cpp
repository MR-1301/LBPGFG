...
Node *solve(int pre[], vector<int> nex, int size, int l, int r) {
    if (l > r)
        return NULL;

    if (l == r)
        return newNode(pre[l]);
    ...
    root->left = solve(pre, nex, size, l + 1, index - 1);
    root->right = solve(pre, nex, size, index, r);
    return root;
}

Node *constructTree(int pre[], int size) {
    stack<int> s;
    vector<int> nex(size, -1);
